# backend.py — V12.3 “Cache-safe” Engine
# ==============================================================================
# Removes @memory.cache from any function that takes st_status (or other
# non-picklable args) to avoid “cannot pickle module” errors.
# Fully aligned with app.py signatures.
# ==============================================================================
...
# Try import pandas_datareader for FRED; if unavailable, skip macro
try:
    import pandas_datareader.data as web
except ImportError:
    web = None

# --- Cache setup (only for pure functions) ---
...
def get_fundamentals_batch(tickers, cache, st_status=None):
    """Fetch & cache fundamentals for each ticker via yfinance."""
    for t in tickers:
        if t in cache:
            continue
        try:
...
def fetch_sp500_constituents(st_status=None):
    """Wrapper around cached `_fetch_sp500_constituents` that ignores st_status."""
    if st_status:
...
def fetch_data(tickers, start, end, st_status=None):
    """Wrapper around cached `_fetch_data` that ignores st_status."""
    if st_status:
