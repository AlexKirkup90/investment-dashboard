# ==============================================================================
# V37 - ENSEMBLE MODEL LIVE ENGINE
# ==============================================================================
# This is the definitive backend for our Streamlit app. It contains all the
# logic for the final Ensemble Model, which combines the High-Return and
# Low-Risk champion models to generate a single, robust portfolio.
# ==============================================================================
...
FUNDAMENTALS_CACHE_FILE = os.path.join(CACHE_DIR, 'fundamentals_ensemble.json')
...
def get_fundamentals(ticker, cache, st_status=None):
    """
    Fetches all required fundamental data for both models.
    """
...
def fetch_market_data(tickers, start, end, st_status=None):
    """
    Fetches market data in smaller chunks to avoid network timeouts.
    """
    ...
    for i in range(0, len(all_tickers), chunk_size):
        ...
        try:
            data = yf.download(chunk, start=start, end=end, auto_adjust=True, timeout=30)
            all_data.append(data)
        except Exception as e:
            print(f"  -> ❌ FAILED to download chunk {i//chunk_size + 1}. Error: {e}")

    if not all_data:
        raise ConnectionError("Failed to download any market data. Check network connection.")

    raw_data = pd.concat(all_data, axis=1)
    prices, highs, lows = raw_data['Close'], raw_data['High'], raw_data['Low']
    return prices, highs, lows

# --- Feature Engineering Functions for Each Model ---
def engineer_features_high_return(prices, monthly_prices, monthly_returns, monthly_spy_returns, sector_mom_1m, sector_map, fundamentals_cache, st_status=None):
    """Generates features for the High-Return Champion model (T1 + E2)."""
    features_dict = {}
...
def engineer_features_low_risk(prices, highs, monthly_prices, monthly_returns, monthly_spy_returns, sector_mom_1m, sector_map, fundamentals_cache, st_status=None):
    """Generates features for the Low-Risk Champion model (T1 + E2 + T3)."""
...
def run_live_prediction_pipeline(st_status):
    ...
    if X_train_hr and X_pred_hr:
        ...
        model_hr = GradientBoostingRegressor(loss='quantile', alpha=0.5, n_estimators=100).fit(scaler_hr.transform(X_train_hr), y_train_hr)
    ...
    X_train_lr, y_train_lr, X_pred_lr, tickers_lr = [], [], [], []
    ...
    if X_train_lr and X_pred_lr:
        ...
        model_lr = GradientBoostingRegressor(loss='quantile', alpha=0.5, n_estimators=100).fit(scaler_lr.transform(X_train_lr), y_train_lr)
        p50_lr = pd.Series(model_lr.predict(scaler_lr.transform(X_pred_lr)), index=tickers_lr)
        
    # --- ENSEMBLE STEP ---
    st_status.text("Combining model predictions and forming portfolio...")
    month_preds = pd.DataFrame({'P50_HR': p50_hr, 'P50_LR': p50_lr})
    month_preds.dropna(inplace=True)
    month_preds['EnsembleScore'] = month_preds[['P50_HR', 'P50_LR']].mean(axis=1)
    
    watchlist = month_preds[month_preds['EnsembleScore'] > 0.005].sort_values('EnsembleScore', ascending=False).head(15)
    
    if len(watchlist) > 1:
        # --- Calculate Equal Weights ---
        num_stocks = len(watchlist)
        equal_weight = 1 / num_stocks
        weights = pd.Series(equal_weight, index=watchlist.index)
